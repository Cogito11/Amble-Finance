...
import { isAssetAccount, isOpenAccount } from "../state/accounts";
...
// Debt accounts are excluded since their balances represent amounts owed, not
// savings, and closed accounts are excluded since they're no longer active.
export function useAccountAmountField({ value, onChange, accounts, balances }) {
  const [mode, setMode] = useState("manual");
  const [accountId, setAccountId] = useState("");
  const savingsAccounts = useMemo(() => (accounts || []).filter((a) => isAssetAccount(a) && isOpenAccount(a)), [accounts]);
